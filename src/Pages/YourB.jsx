import React from 'react'

function YourB() {
  return (
	<>
	
	<div className="bg-gray-100 p-4 rounded w-full max-w-8xl mx-auto  relative flex-nowrap min-h-fit">
		<div className="flex justify-between">
			<h2 className='font-semibold'>Your browsing history</h2>
		<h2 className='hover:underline text-xs text-blue-400 '>View or edit your browsing history</h2>
		</div>
		
		<div className="w-full h-45 object-contain rounded flex overflow-x-auto  ">
	<img src="https://storage.googleapis.com/fir-auth-1c3bc.appspot.com/1692255251854-xbox.jpg" alt="" />
<img src="https://storage.googleapis.com/fir-auth-1c3bc.appspot.com/1692947383286-714WUJlhbLS._SL1500_.jpg" alt="" />
<img src="https://images-eu.ssl-images-amazon.com/images/G/31/IMG15/Irfan/GATEWAY/MSO/186x116---wm._SY116_CB667322346_.jpg" alt="" />
<img src="https://images-eu.ssl-images-amazon.com/images/G/31/IMG15/Irfan/GATEWAY/MSO/Appliances-QC-PC-186x116--B08RDL6H79._SY116_CB667322346_.jpg" alt="" />
<img src="https://m.media-amazon.com/images/I/71RvZs6LY+L.AC_SY200.jpg" alt="" />
<img src="https://m.media-amazon.com/images/I/71UzlmK-W1L._AC_SY200_.jpg" alt="" />
<img src="https://m.media-amazon.com/images/I/51+b9IWWbjL.AC_SY200.jpg" alt="" />
<img src="https://m.media-amazon.com/images/I/71s+auflKiL._AC_SY200_.jpg" alt="" />
		</div>
	</div>
	</>
  )
}

export default YourB